...
const storedStateDefaults = {
  policy: defaultPolicy,
  logBudget: defaultLogBudget,
  spikeAlerts: defaultSpikeAlerts
} as const;
...
export default function App() {
  const [initialStoredState] = useState<Partial<StoredState> | null>(() =>
    readStoredState(storageKey, storedStateDefaults)
  );
  ...
  const importState = (file: File | null) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      const parsed = sanitizeStoredState(
        safeJsonParse(String(reader.result)),
        storedStateDefaults
      );
      if (!parsed) {
