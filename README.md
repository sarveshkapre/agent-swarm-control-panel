...
## Status
MVP UI with approvals drawer, policy editor (including emergency stop for pausing new runs), log filters/pinning, growth momentum dashboard, run health summary with escalation draft copying, saved run templates/playbooks (CRUD, local-only), Integration Hub sync health telemetry (local-only), and local state import/export. Evidence exports include SHA-256 integrity metadata, and run-duration/SLA badges are computed from run timestamps/event data (not static seeds). No authentication (local-only).
